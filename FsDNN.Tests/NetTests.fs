...
let ``makeLayer - logistic unit`` () =
  ...
  it.LossGraphString |> should equal "BCEWithLogitsLoss[Loss,1]( Y[TG=false], Sigmoid( Add1( Multiply1( W1[TG=true], X[TG=false] ), b1[TG=true] ) ) )"
  it.PredictGraphString |> should equal "Linear[Predict,1]( Sigmoid( Add1( Multiply1( W1[TG=true], X[TG=false] ), b1[TG=true] ) ) )"
...
let ``makeLayer - multilayer perceptron`` () =
  ...
  let it = n |> Net.toString
  it.LossGraphString |> should equal "CCEWithLogitsLoss[Loss,2]( Y[TG=false], Linear( Add2( Multiply2( W2[TG=true], Sigmoid( Add1( Multiply1( W1[TG=true], X[TG=false] ), b1[TG=true] ) ) ), b2[TG=true] ) ) )"
  it.PredictGraphString |> should equal "HardMax[Predict,2]( Linear( Add2( Multiply2( W2[TG=true], Sigmoid( Add1( Multiply1( W1[TG=true], X[TG=false] ), b1[TG=true] ) ) ), b2[TG=true] ) ) )"
